		...
		<div id="wrapA">
			<div id="header">
				<div class="toptool tar">
					<span><a href="<%=basePath %>index.jsp">Home首页</a> | <a
						href="<s:url action='list' namespace='/category' />">论坛首页</a> </span>
				</div>
				<table cellspacing="0" cellpadding="0" align="center" width="100%">
					<tr>
						<td class="banner">
							<a href="<s:url action='index' namespace='/' />"><img src="../images/yellow/logo.png" /> </a>
						</td>
				...
				<div id="h_guide" class="guide" colspan="2">
					<span class="s3">&raquo;</span>
						...
						<s:if test="#session.user == null">
							您尚未&nbsp;
							<a href="<s:url action='init-login' namespace='/person'/>">登录</a> |
							<a href="<s:url action='init-add' namespace='/person'/>">注册</a>
						</s:if>
						<s:else>
							欢迎您, <a
								href="<s:url action='viewPerson' namespace='/person'/>?person.id=${sessionScope.user.id}"><s:property value="#session.user.account"/></a> 
									| <a href="<s:url action='viewReplies' namespace='/person'/>?person.id=${sessionScope.user.id}">
									回复我的文章<s:if test="#session.user.news != 0"><span style="color:red">(<s:property value="#session.user.news"/>)</span></s:if></a> 
								| <a
								href="<s:url action='logout' namespace='/person'/>">注销</a>
						</s:else>
				</div>
			</div>
			<div id="main">
