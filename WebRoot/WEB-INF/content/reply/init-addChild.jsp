...
<div id="main">
				...
				<td style="padding-top: 12px;text-align: left;">
					...
					<b> <a
						...
						href="<s:url action='list' namespace='/board' />?board.id=${thread.board.id}">${
							thread.board.name }</a> &raquo; 回复 <a
	...
	<s:form action="addChild" namespace="/reply" onsubmit="return check();">
		<s:hidden name="reply.id" value="%{reply.id}" />
		<s:hidden name="childReply.title" value="Re: %{thread.title}" />
		<div class="t t2">
			<table cellspacing="0" cellpadding="0" width="100%"
				...
				<tr class="tr1 r_one">
					...
					<th style="vertical-align:bottom;padding-left:15px;border:0">
						<s:textarea name="childReply.content" id="con" onclick="check()"
							cssStyle="width: 500px; height: 200px; " 
							value="<p>&nbsp;</p>
							<p>【 在%{reply.author.account}中的大作中提到】:</p>
							%{reply.content}"></s:textarea>
						<br />
						<br />
