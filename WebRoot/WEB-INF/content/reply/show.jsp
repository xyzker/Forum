...
<div id="main">
		...
		<table cellspacing="0" cellpadding="0" width="100%" align="center">
			<tr>
				<td style="text-align: left;">
					...
					<b> <a
						href="<s:url action='viewCategory' namespace='/category' />?category.id=${reply.thread.board.category.id}">${
							reply.thread.board.category.name }</a> &raquo; <a
						href="<s:url action='list' namespace='/board' />?board.id=${reply.thread.board.id}">${
							reply.thread.board.name }</a> </b>
				</td>
			</tr>
		</table>
	</div>
		...
		<table width="100%" align="center" cellspacing="0" cellpadding="0">
			<tr>
				<td align="right" style="padding-bottom:.2em">
					<a
						href="<s:url action='init-add' namespace='/reply'/>?thread.id=${reply.thread.id}"><img
							src="<%=basePath %>images/yellow/reply.png" /> </a>
					<a
						href="<s:url action='init-add' namespace='/thread'/>?board.id=${reply.thread.board.id}"
	...
	<div class="t" style="margin-bottom:0px; border-bottom:0">
		<table cellspacing="0" cellpadding="0" width="100%">
			<tr>
				<th class="h">
					<strong class="fl w"><b>本页主题:</b> ${ reply.title }</strong>
				</th>
			</tr>
			<tr>
				<td></td>
			...
			<tr>
				<td align="left">
					<th style="text-align: center; line-height: 20px;">
						<a href="<s:url action='list' namespace='/thread'/>?thread.id=${reply.thread.id}" style="color: blue">查看原帖</a>
					</th>
					<th style="text-align: center; line-height: 20px; ">
						<a href="javascript:window.history.go(-2);" style="color: blue">返回</a>
					</th>
				</td>
				<td align="right" style="padding-bottom:.2em">
					<a
